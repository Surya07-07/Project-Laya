...
class AgentBrain:
    ...
    def think(self, request):
        ...
        tool = self.router.route(intent["intent"])
        ...
        while True:
            ...
            result = self.retry.run(self.executor.execute, step["tool"], step["data"])

            check = self.verify.verify(result["result"])

            results.append({"execution": result, "verification": check})

            self.context.update(request, tool, result)

        memory = self.memory.save_success(request, intent["intent"], results)
